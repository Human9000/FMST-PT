...
def tr_epoch(model, device, loader, optimizer, loss_func, metricsLoss, metricsDice, metricsDiceRes):
    model.train()
    ...
    for item in tqdm(loader, total=len(loader)):
        ...
        attn = item['attn'].to(device)[:,1:]
        y = item['label'].to(device)

        optimizer.zero_grad()  # 重置梯度
        output = model(x,attn)  # 模型求解
...
def val_epoch(model, device, loader, loss_func, metricsLoss, metricsDice, metricsDiceRes):
    model.eval()
    ...
    for item in tqdm(loader, total=len(loader)):
        ...
        y = item['label'].to(device)
        output = model(x,attn)  # 模型求解
        save(torch.cat((x, x, x), dim=1),
             f"val/attention/{item['conf']['name'][0]}_x.png")
        save(torch.cat((output[0][:, 1:], output[0]), dim=1),
             f"val/attention/{item['conf']['name'][0]}_pre.png")
        save(torch.cat((y[:, 1:], y[:, 1:], y[:, 1:]), dim=1),
             f"val/attention/{item['conf']['name'][0]}_y.png")
        loss = loss_func(output[0], y)
        for i in range(len(output)-1, 0, -1):
            rrSidualC = 1.5 - 1/(1 + 2.7**(-i))  # 权重系数
            loss += rrSidualC * loss_func(output[i], y)
...
def main():
    ...
    # 超参数配置
    # loss_func = loss.AttnLoss(0.4)  # 损失函数
    ...
    for epoch in range(2000):
        ...
        tr_log = tr_epoch(model, device, tr_loader, optimizer,
                          loss_func, metricsLoss, metricsDice, metricsDiceRes)
