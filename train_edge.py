...
def tr_epoch(model, device, loader, optimizer, loss_func, metricsLoss, metricsDice, metricsAttnDice):
    model.train()
    ...
    for item in tqdm(loader, total=len(loader)):
        ...
        metricsDice.update(output, y)
        metricsAttnDice.update(output, y)

    return OrderedDict({'loss': metricsLoss.avg,
                        'dice0': metricsDice.avg[0],
                        'dice1': metricsAttnDice.avg[1],
                        'attndice0': metricsAttnDice.avg[1],
                        'attndice1': metricsAttnDice.avg[1],
                        })


def val_epoch(model, device, loader, loss_func, metricsLoss, metricsDice, metricsAttnDice):
    model.eval()
    ...
    for item in tqdm(loader, total=len(loader)):
        ...
        save(x,   f"val/edge/{item['conf']['name'][0]}_x.png")
        save(output[:, 1:],     f"val/edge/{item['conf']['name'][0]}_pre.png")
        save(y[:, 1:],        f"val/edge/{item['conf']['name'][0]}_y.png")
...
def main():
    ...
    ckpt = torch.load('save/edge/best_edge.pth')
    ...
    for epoch in range(2000):
        ...
        tr_log = tr_epoch(model, device, tr_loader, optimizer,
                          loss_func, metricsLoss, metricsDice, metricsAttnDice)
